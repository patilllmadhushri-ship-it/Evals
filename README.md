...
**Judge indirection.** All LLM-based scoring routes through a single configurable judge client ([`stt_eval/judge.py`](stt_eval/judge.py)), so the judge model swaps without touching any metric. Two backends ship:

- **Anthropic** — Claude directly, with server-enforced JSON schemas.
- **OpenRouter** — any model on the OpenRouter catalogue, including reasoning models (DeepSeek R1, o3-mini, Gemini 2.5 Pro, QwQ, Grok 4, or any slug you type in). Reasoning depth maps to OpenRouter's unified `reasoning.effort`, so one setting works across model families, and OpenRouter reports the real cost of each call — the judge spend shown after a run is actual, not estimated.

Reasoning models are worth the extra latency here: judging whether *"transfer it to the savings account"* versus *"transfer it from the savings account"* preserves meaning is exactly the kind of call a fast model gets wrong. The OpenRouter client asks for a strict JSON schema, falls back to a JSON-only instruction for models that don't support one, and parses tolerantly — fenced blocks and stray preambles from reasoning models are handled rather than failing the metric.

**The judge's system prompt** lives in one place, [`stt_eval/prompts.py`](stt_eval/prompts.py), shared by every metric and every backend so a verdict means the same thing regardless of who issued it. It's written against how recognisers actually fail: numerals versus number words, abbreviations, fillers, transliteration and regional spelling are explicitly *not* errors, while entity corruption (one wrong digit in a phone number), negation flips, meaning-changing homophones, dropped clauses, hallucinated text on silence, and translating instead of transcribing explicitly *are*. You can read the exact prompt in step 3, and edit it in that file.
...
readnet/                    Akshar / PadhAI: a child's reading -> ASER level (see readnet/README.md)
```
...
`smoke_test.py` synthesises audio, runs the mock provider through the real runner, and checks alignment, validation, resumability, aggregation and export. `ui_test.py` renders every step of the Streamlit app headlessly. None of the three needs an API key or network access; the Akshar suite also runs in CI on every push that touches `readnet/`.
